...
function varargout=qfiscgen(varargin)
    ...
    mfinnames = [];
    mfoutnames=[];
%     mfoutnames = strings( 1, length([fis.outputs(:).membershipfunctions] ) );
    codeparams = "";
    codemfsetup = tabchar + "/* Add membership functions to the inputs */" + newline;
    codeiosetup = tabchar + "/* Add inputs */" + newline;
    coderun1 = "";
    coderun2 = "";
    coderulew = "";
    ...
    for k=1:nins
        ...
        codeiosetup = codeiosetup + tabchar + "qFIS_SetIO( " + fuzzin_arrname + ", " + innames{k} + ", " +  sprintf("%.4ff",fis.input(k).range(1)) + ", " + sprintf("%.4ff",fis.input(k).range(2)) + " );" + newline;
        ...
        coderun1 = coderun1 + tabchar + fuzzin_arrname + "[ " + innames{k} + " ].value = inputs[ " + innames{k} + " ]" + newline;
    end
    codemfsetup = codemfsetup + tabchar + "/* Add membership functions to the outputs */" + newline; 
    outnames = strings(1,length(fis.outputs));
    for k=1:nouts
        fis.outputs(k).name=fix2varname(fis.outputs(k).name);
        outnames(k) = fis.outputs(k).name;
        codeiosetup = codeiosetup + tabchar + "qFIS_SetIO( " + fuzzout_arrname + ", " + outnames{k} + ", " + sprintf("%.4ff",fis.output(k).range(1)) + ", " + sprintf("%.4ff",fis.output(k).range(2)) + " );" + newline;
        ...
        coderun2 = coderun2 + tabchar + "outputs[ " + outnames{k} + " ] = " + fuzzout_arrname + "[ " + outnames{k} + " ].value;" + newline;
    end
    ...
    if rw ~= length(fis.rules)
        ...
        srw(end:-1:end-1)='';
        coderulew = coderulew + "static const float ruleWeights[] = { " + srw + " };" + newline;
    end
    ...
    code = '#include "' + objname + '_fis.h"' + newline + ... 
           ...
           "static qFIS_IO_t " + fuzzin_arrname + "[ " + num2str(nins) + " ], " + fuzzout_arrname + "[ " + num2str(nouts) + " ];" + newline + ...
    ...
    code = code + "}" + newline + newline + ...
           ...
           tabchar + "if ( qFIS_Inference( &" + objname + ", rules ) > 0 ) {" + newline + ...
